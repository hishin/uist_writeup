...
\item{\textit{Consistency with the other text}: Since the end goal is to align the segments from both texts, we would like the segment boundaries from one text to align with the segment boundaries in the other text. Along with the binary match output, NW also a mapping between the . \VTODO{Given a segmentation of one text, we define the consistency score of the other text as follows: }} 
\end{enumerate}
