...
Ideally, our segments would respect natural boundaries such as sentence punctuations, line breaks and pauses in the audio. We would also like to separate parts of the transcript that agree with the master-script (i.e. planned speech) from parts that do not (i.e. improvised speech).  
...
\VTODO{How to match these segments.}

We use this output in the \textit{compare-view} to display matching segments side-by-side and to overwrite a segment of the  master-script when the user accepts a matching transcript segment.

\VTODO{Are the segments used anywhere else? Given that this is the main technical piece of our approach, it seems like we should emphasize what aspects of our interface it enables. Doesn't the All tab also rely on this segmentation? }