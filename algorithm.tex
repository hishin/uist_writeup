...
For each optimization step, we use the classic optimal line-breaking algorithm by Knuth and Plass \cite{}. Given a sequence of $n$ words $T = \{w_0,\dots,w_n\}$, the algorithm finds the optimal set of inter-word
boundaries that break the text into segments. We refer to the boundary between $w_i$ and $w_{i+1}$ as
$b_i$.
%
The algorithm iterates through each word in order, and for each $w_i$
computes and records the optimal set of text segments $S_i$ formed
by all strokes up to $b_i$, along with the total score $E(S_i)$ of
...
segmenting the words $T_{ji} = \{w_\text{j+1},
